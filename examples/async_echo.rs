...
fn main() {
    ...
    let tp = TransportBuilder::new(ctx.clone()).create().expect("Couldn't create default transport.");
    ...
    let events = incoming.and_then(|mut msg| { // and_then applies some function to each element of the stream, passing the stream onward
        ...
        // Do some useful work...
        Ok(msg)
    }).forward(tp).then(|_| Ok(())); // Forward the `Stream` of futures to this `Sink`, in our case the default transport.
